...
export const AuthProvider = ({ children }) => {
  ...
  // const [login, setLogin] = useState(theUser);
  ...
  const loggedInCheck = async () => {
    ...
    // clienteAxios.defaults.headers["content-type"] = "application/json";
    ...
    if (loginToken) {
      ...
      const { rol, email, activado, name } = parseToken.data;
      console.log(name);
      ...
      if (data) {
        // alert('exito login')
                 setUser({"email": `${email}`, "role": `${rol}`, "name": `${name}`, "active": `${activado}`});
                 if (activado === 0 || rol === '') {
                  localStorage.removeItem("loginToken");
                  setUser(null);
                  window.location.href = `/noautorizado`;
